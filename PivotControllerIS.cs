...
// version 2022-03-02, Input System version
//
...
//   - Create a "Pivot Controller" Input system action map
//       - Roll, Yaw, Zoom (1D Axis)
//       - Fine Movement (Button)
//
...
public class PivotControllerIS : MonoBehaviour
{
    ...
    Controls controls;
    private bool isHeldRoll;
    private bool isHeldYaw;
    private bool isHeldZoom;
    private bool isPressedRoll;
    private bool directionRoll;
    private bool directionYaw;
    private bool directionZoom;
    ...
    {
        controls.PivotController.Enable();
        //read rotation on up-down axis (roll)
        controls.PivotController.Roll.performed += OnRollHeld;
        controls.PivotController.Roll.canceled += OnRollCanceled;
        //read roatation on left-right axis (yaw)
        controls.PivotController.Yaw.performed  += OnYawHeld;
        controls.PivotController.Yaw.canceled   += OnYawCanceled;
        //read zoom in/out
        controls.PivotController.Zoom.performed += OnZoomHeld;
        controls.PivotController.Zoom.canceled  += OnZoomCanceled;
        //read slow/standard movement speed
        controls.PivotController.FineMovement.performed += OnSpeedHeld;
        controls.PivotController.FineMovement.canceled  += OnSpeedCanceled;
    }

    private void OnRollHeld(InputAction.CallbackContext context)
    {
        if (restrictedRollMovement)
        {
            isPressedRoll = true;
        ...
        directionRoll = context.ReadValueAsButton();
    }
    ...
    private void OnYawHeld(InputAction.CallbackContext context)
    {
        isHeldYaw = true;
        directionYaw = context.ReadValueAsButton();
    }
    ...
    private void OnZoomHeld(InputAction.CallbackContext context)
    {
        isHeldZoom = true;
        directionZoom = context.ReadValueAsButton();
    }
    ...
    private void OnSpeedHeld(InputAction.CallbackContext context)
    {
    ...
    {
        //debug
        //Debug.Log("Roll: " + isHeldRoll + ". Yaw: " + isHeldYaw + ". Zoom : " + isHeldZoom + ".");

        //get movement input (yaw rotation)
        if (isHeldYaw && directionYaw)
        {
        ...
        if (!restrictedRollMovement && isHeldRoll && directionRoll)
        {
        ...
        else if (!restrictedRollMovement && isHeldRoll)
        {
        ...
        if (restrictedRollMovement && isPressedRoll && directionRoll)
        {
            setRollSegment(currentRollSegment + 1);
            isPressedRoll = false;
        }
        else if (restrictedRollMovement && isPressedRoll)
        {
            setRollSegment(currentRollSegment - 1);
            isPressedRoll = false;
        }
        //get zoom input
        if (isHeldZoom && directionZoom)
        {
